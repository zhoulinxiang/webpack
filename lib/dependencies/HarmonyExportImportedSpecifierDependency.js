...
/** @typedef {"missing"|"unused"|"empty-star"|"reexport-non-harmony-default"|"reexport-named-default"|"reexport-namespace-object"|"reexport-non-harmony-default-strict"|"reexport-fake-namespace-object"|"reexport-non-harmony-undefined"|"normal-reexport"|"dynamic-reexport"} ExportModeType */
...
class HarmonyExportImportedSpecifierDependency extends HarmonyImportDependency {
	...
	_getErrors(moduleGraph) {
		...
		if (!importedModule.buildMeta || !importedModule.buildMeta.exportsType) {
			...
			if (
				moduleGraph.getParentModule(this).buildMeta.strictHarmonyModule &&
				(this.ids.length === 0 || this.ids[0] !== "default")
			) {
				// In strict harmony modules we only support the default export
				const exportName =
					this.ids.length > 0
						? `the named export ${this.ids.map(id => `'${id}'`).join(".")}`
		...
		if (moduleGraph.isExportProvided(importedModule, this.ids) !== false) {
			// It's provided or we are not sure
			return;
		}
		console.log(this.ids, importedModule.identifier(), this.name);

		// We are sure that it's not provided
		const idIsNotNameMessage =
			this.ids.join(".") !== this.name ? ` (reexported as '${this.name}')` : "";
